...
After installation, use **Settings -> SDDM** to preview and deploy changes. The page supports a current or separately selected theme, theme/custom font, current or alternate wallpaper from the shared `.thumbs` library, full time/date appearance controls, clock shadows, and login-panel sizing/positioning. **Test SDDM Theme** previews current unsaved values from a temporary copy without root writes; **Apply to SDDM** performs the real hash-aware installation. Desktop changes do not automatically write root-owned files.
...
Do not add a fixed `size` line. Set the preferred width and height under **Settings -> Appearance -> Settings Window**.
