...
//=============================================================================
//
// 2026-07-19  (GPT) Rev 29 final major settings split: extracted the
//             page-facing compatibility facade, option models, overlay state,
//             and SettingsTransaction host into SettingsContext.qml. The
//             FloatingWindow now owns lifecycle and native window behavior.
...
FloatingWindow {
    ...
    // GPT Rev 29: page-facing settings state and compatibility API live in
    // SettingsContext. The window now owns only lifecycle/native behavior.
    SettingsContext {
        id: settingsContext
        window: root
    }
    ...
    function close(): void {
        shown = false;
        settingsContext.discardStaged(); // close discards — see DESIGN NOTES
    }
